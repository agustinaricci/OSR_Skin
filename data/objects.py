...
from config import objects_root
import argparse

def create_val_img_folder(path_data_in='data/OBJECTS'):
    '''
    This method is responsible for separating Object images into separate sub folders
    path_data_in points to a folder containing the 3 subfolders
    '''
    path_data = path_data_in.replace('OBJECTS', 'OBJECTS')
...
class Objects(torchvision.datasets.ImageFolder):

    def __init__(self, root, transform):

        super(Objects, self).__init__(root, transform)
...
def get_objects_datasets(train_transform, test_transform, known_classes=(0,1), open_set_classes=(2), seed=0):

    objects_train_dir = osp.join(objects_root, 'train')
    objects_val_dir = osp.join(objects_root, 'val')
    objects_test_dir = osp.join(objects_root, 'test')

    np.random.seed(seed)
    # Build train/val dataset and subsample known_classes
    train_dataset = Objects(root=objects_train_dir, transform=train_transform)
    train_dataset = subsample_classes(train_dataset, include_classes=known_classes)

    val_dataset = Objects(root=objects_val_dir, transform=test_transform)
    ...
    test_dataset_known = Objects(root=objects_test_dir, transform=test_transform)
    test_dataset_known = subsample_classes(test_dataset_known, include_classes=known_classes)

    # Build test dataset and subsample known/unknown classes
    test_dataset_unknown = Objects(root=objects_test_dir, transform=test_transform)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path_data_in', type=str, default='data/OBJECTS', help="")
